...
const fwbData = (result) => {
	let recommendList = [];
	let isGoods = false;
	result.content = result.content.replace(/\<img(.*?)src/g,
		'<img style="max-width:100%!important;height:auto;display:block;" src'); // 图片适配
	result.content = result.content.replace(/\<p/g,
		'<p style="line-height:1.8;text-indent:25px;word-break: break-all;"'); // p标签样式
	result.content = result.content.replace(/\<font color="/g, '<font style="color:'); // 字体颜色转换
	result.content = result.content.replace(
		/\<p style="line-height:1.8;text-indent:25px;word-break: break-all;"\>\<img/g, "<p><img");
	let contentArr = [];
	let contentArr2 = [];
	if (result.content.indexOf("<shop>") != -1 || result.content.indexOf("<goods>") != -1 || result.content.indexOf(
			"<activity>") != -1) {
		while (result.content.indexOf("<shop>") != -1 || result.content.indexOf("<goods>") != -1 || result.content
			.indexOf("<activity>") != -1) {
